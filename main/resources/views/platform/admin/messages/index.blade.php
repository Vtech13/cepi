        ...
        <div class="block-content">
            ...
            <ul>
                @foreach ($groups as $group)
                    <li>
                        <a href="{{ route('admin.messages.show', $group) }}">{{ $group->name }} | Objet : {{ $group->objet }}</a>
                    </li>
                @endforeach
            </ul>

            <h2>Nouveau Groupe</h2>
            <form action="{{ route('admin.groups.store') }}" method="POST" class="form">
                @csrf
                ...
                <div class="form__group">
                    <label for="objet">Objet du groupe:</label>
                    <input type="text" class="form__input objet" name="objet" id="objet">
                </div>
