 ...
 * MENU
 * ======================================================================== */
const navToggle = document.getElementById('icon-nav-toggle');
...
function openNav() {
    ...
    if (classList.hasClass(navLinks, 'nav-open')) {
        classList.removeClass(document.querySelector('.wrap'), 'toggled-nav');
        classList.removeClass(navToggle, 'open');
        classList.removeClass(navLinks, 'nav-open');
    } else {
        classList.addClass(document.querySelector('.wrap'), 'toggled-nav');
        classList.addClass(navToggle, 'open');
        classList.addClass(navLinks, 'nav-open');
    }
}
...
function dropdown() {
    if (checkIfMobileOrTablet()) {
        for (const dropdownBtn of document.querySelectorAll('.dropdown')) {
            dropdownBtn.addEventListener('click', function (e) {
                if (classList.hasClass(e.target, 'dropdown__button')) {
                    e.preventDefault();
        ...
        for (const dropdownBtn of document.querySelectorAll('.dropdown')) {
            ...
            dropdownBtn.addEventListener('mouseout', function (e) {
                e.preventDefault();
                this.querySelector('.dropdown__content').style.display = 'none';
            });
        }
...
if (classList.hasClass(document.body, 'contact')) {

    maps();
}

function maps() {
    const coordinates = [47.6853975, 2.6338269];
    const myMap = L.map('mapid', {scrollWheelZoom: false}).setView(coordinates, 13);
