...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var pusher = new Pusher(window.PusherConfig.key, {
    ...
    // Add this line to set the auth endpoint
    auth: {
      headers: {
  ...
  channel.bind('new-message', function (data) {
    ...
    if (Notification.permission === "granted") {
      ...
      console.log('Notification créée :', notification); // Vous pouvez également ajouter un écouteur d'événements pour que quelque chose se produise lorsque l'utilisateur clique sur la notification
    ...
    var messagesContainer = document.querySelector('.message-list'); // Determine if the current user is the sender

    var isSender = data.message.user_id.toString() === window.userId.toString(); // Create the list item for the new message

    var newMessageElement = document.createElement('li');
    newMessageElement.className = "message ".concat(isSender ? 'sender' : 'receiver');
    var messageContent = document.createElement('div');
    messageContent.className = 'message-content'; // Create the user name element

    var userNameElement = document.createElement('small');
    ...
    messageContent.innerHTML += "<p>".concat(data.message.content, "</p>");

    if (data.message.file_path) {
      var fileLink = document.createElement('a'); // Remove the incorrect prefix from the file path

      var correctFilePath = data.message.file_path.replace('public/', '/storage/');
    ...
    var day = '0' + messageDate.getDate();
    var month = messageDate.getMonth() + 1; // Les mois sont indexés à partir de 0
    ...
    var strTime = day + '-' + month + '-' + year + ' ' + hours + ':' + minutes + ':' + seconds + ' ' + ampm;
    timeSpan.textContent = strTime;
    messageContent.appendChild(timeSpan);
    newMessageElement.appendChild(messageContent); // Add the new message to the top of the list
